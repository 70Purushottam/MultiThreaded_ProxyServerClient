...
static DWORD WINAPI pthread_start_routine(LPVOID param)
{
    pthread_start_info* info = (pthread_start_info*)param;
...
static int pthread_create(pthread_t* thread, const void* attr, void* (*start_routine)(void*), void* arg)
{
    ...
    info->arg = arg;
    *thread = CreateThread(NULL, 0, pthread_start_routine, info, 0, NULL);
...
#define inet_ntop(af, src, dst, size) inet_ntop_compat((af), (src), (dst), (size))
#endif
